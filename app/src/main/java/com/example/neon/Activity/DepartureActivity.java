...
//                Toast.makeText(SecondActivity.this, data.get(position).getFlightNumber(),Toast.LENGTH_SHORT).show();
                String jsonFlight = new Gson().toJson(data.get(position));

                editor.putString("flight", jsonFlight);
                editor.commit();

                Intent intent = new Intent(getApplicationContext(), FlightDetailActivity.class);
                String jsonServicesList = new Gson().toJson(data.get(position).getServicesList());
                intent.putExtra("flight_url", jsonServicesList);
